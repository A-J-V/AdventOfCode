"""
-----------------------------------------------------------------
PART 2 is the same as Part 1, except for the following difference:
After repeating steps 1-11 below, the final target value is saved, all wire values are reset,
b is assigned the saved value of the final target value, and steps 9-11 are repeated.
-----------------------------------------------------------------
...
# All gates have been activated. Save the final value of the target wire. 
run()
initial_final = wire_dict['a']
for wire in wire_dict.keys():
    wire_dict[wire] = None
wire_dict['b'] = initial_final
run()
print(f"The updated final value of 'a' for part 2 is {wire_dict['a']}.")
