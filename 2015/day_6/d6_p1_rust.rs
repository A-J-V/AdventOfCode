    ...
    for y in y1..y2+1 {
        for x in x1..x2+1 {
            match instruction {
                ...
                "off " => {light_array[DIM * y as usize + x as usize] = -1},
                "toggle " => {light_array[DIM * y as usize + x as usize] *= -1},
                _ => (),
            }
        }
    }
...
fn main() {
    ...
    let on_lights: i32 = light_array.iter().filter(|x| **x == 1).sum();
    println!("There are {} lights on!", on_lights);
